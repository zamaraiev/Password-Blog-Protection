...
function bpp_settings_page_html(){
    ...
    if(isset($_POST['password_protection_password'])) {
        ...
        if(!empty($_POST['password_protection_password'])) {
            ...
            $cookie_session_lifetime = intval($_POST['password_protection_cookie_session_lifetime']);
            update_option('password_protection_cookie_session_lifetime', $cookie_session_lifetime);

            //allow share access
            $share_access = isset($_POST['password_protection_share_access']) ? '1' : '0';
            update_option('password_protection_share_access', $share_access);

            // Update categories
            $enable_protection_for_certain_categories_array = isset($_POST['password_protection_for_certain_categories']) ? array_map('sanitize_text_field', $_POST['password_protection_for_certain_categories']) : [];
            update_option('password_protection_for_certain_categories', $enable_protection_for_certain_categories_array);

            // Update certain user roles
            $disable_protection_for_certain_user_roles_array = isset($_POST['password_protection_for_certain_user_roles']) ? array_map('sanitize_text_field', $_POST['password_protection_for_certain_user_roles']) : [];
            update_option('password_protection_for_certain_user_roles', $disable_protection_for_certain_user_roles_array);

            // Enable protection for blog page
            $blog_page_protection = isset($_POST['password_protection_for_blog_page']) ? '1' : '0';
            update_option('password_protection_for_blog_page', $blog_page_protection);

            // Update restricted content message
            $restricted_content_message = sanitize_text_field($_POST['password_protection_resctircted_content_message']);
            update_option('password_protection_resctircted_content_message', $restricted_content_message);

            // Update restricted content message for feeds
            $restricted_content_message_feeds = sanitize_text_field($_POST['password_protection_resctircted_content_message_feeds']);
            update_option('password_protection_resctircted_content_message_feeds', $restricted_content_message_feeds);
				
            // Update 'Return Back' link settings
			$return_back_link_url = sanitize_text_field($_POST['password_protection_return_back_link_url']);
			update_option('password_protection_return_back_link_url', $return_back_link_url);
				
            // Update 'Return Back' link text
			$return_back_link_text = sanitize_text_field($_POST['password_protection_return_back_link_text']);
			update_option('password_protection_return_back_link_text', $return_back_link_text);
			
			// Update incorect password(error) message
			$error_message_field = sanitize_text_field($_POST['password_protection_error_message']);
            update_option('password_protection_error_message', $error_message_field);
        } 
        else {
    ...
    $restricted_content_message = get_option('password_protection_resctircted_content_message', 'This content is password protected. Please enter the password below to access it.'); // 'This content is password protected. Please enter the password below to access it.' for default
    $restricted_content_message_feeds = get_option('password_protection_resctircted_content_message_feeds', 'This content is password protected. Please enter the password below to access it.');  // 'This content is password protected. Please enter the password below to access it.' for default
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="number" name="password_protection_cookie_session_lifetime" value="<?php echo esc_attr( $cookie_session_lifetime); ?>" />
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="checkbox" name="password_protection_for_blog_page" value="1" <?php checked('1', $enable_protection_for_blog_page); ?> />
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="checkbox" name="password_protection_share_access" value="1" <?php checked('1', $is_share_access_turned_on); ?> />
                        <?php
                         // Display link for sharing access
                        if ($is_share_access_turned_on === '1' && !empty($password)) {
                            $ciphered_password = bpp_cipher($password, 'mySecretSalt');
                            echo '<b>You can use this URL to share access to the blog without password: ' 
                                . esc_url(get_permalink(get_option('page_for_posts'))) . '#' . $ciphered_password . '</b>';
                        }
                    ?>
                    </td>
                </tr>
				...
				<tr valign="top">
                    ...
                    <td>
                        <input type="text" name="password_protection_return_back_link_url" value="<?php echo esc_attr($return_back_link_url); ?>"/>
						<p>"Return Back" link url</p>
                        <input type="text" name="password_protection_return_back_link_text" value="<?php echo esc_attr($return_back_link_text); ?>"/>
                ...
                <tr valign="top">
                    ...
                    <td>
                        <textarea name="password_protection_resctircted_content_message" rows="4" cols="50"><?php echo esc_attr($restricted_content_message); ?></textarea>
                        <p>Message shown to users when accessing password-protected content.<p>
                        <textarea name="password_protection_resctircted_content_message_feeds" rows="4" cols="50"><?php echo esc_attr($restricted_content_message_feeds); ?></textarea>
                        ...
                        <p>By default "Incorrect password. Try again."<br>
                    </td>
                </tr>
            </table>
