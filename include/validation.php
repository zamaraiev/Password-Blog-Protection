...
class Validation_DMYZBP {
    ...
    public function __construct($plugin_settings) {
        add_action( 'wp_ajax_dmyzbp_check_password', [ $this, 'ajax_validate_password' ] );
        add_action( 'wp_ajax_nopriv_dmyzbp_check_password', [ $this, 'ajax_validate_password' ] );
        add_action( 'wp_ajax_dmyzbp_check_cookie', [ $this, 'ajax_validate_access_cookie' ] );
        add_action( 'wp_ajax_nopriv_dmyzbp_check_cookie', [ $this, 'ajax_validate_access_cookie' ] );
        $this->settings = $plugin_settings;
    }

    /* Add AJAX action to validate password */
    public function ajax_validate_password() {
        check_ajax_referer('dmyzbp_ajax_nonce');

        $entered_password = isset($_POST['password']) ? sanitize_text_field( wp_unslash($_POST['password']) ) : ''; // Get password from AJAX request
    ...
    public function validate_access_cookie() {
        $cookie_value = isset( $_COOKIE['blog_access'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['blog_access'] ) ) : '';
